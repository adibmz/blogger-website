...
// Use EJS as the view engine and resolve views via an absolute path
...
app.get("/", (req, res) => {
  // render by view name (no .ejs extension)
  res.render("index", { arr: arr, length: arr.length });
});
...
// Only start a persistent listener in non-production (local dev).
// On Vercel we export the app so it is mounted into a serverless function.
